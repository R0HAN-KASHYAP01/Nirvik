...
class _CameraScreenState extends State<CameraScreen> {
  ...
  Future<void> _loadFeeds() async {
    ...
    try {
      final feeds =
          await NgoCameraService.instance.fetchFeeds(
        user.id,
      );
  ...
  Future<void> _submitLiveLink() async {
    ...
    try {
      ...
      setState(() {
        _linkError =
            'Could not add link. Please try again.';
      });
    } finally {
      if (mounted) {
        setState(() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            controller:
                _uploadLabelController,
            decoration:
                InputDecoration(
              hintText:
                  'e.g. Main Hall Camera',
              hintStyle:
                  const TextStyle(
                fontSize: 12,
                color:
            ...
            ElevatedButton(
              onPressed: _submittingLink
                  ...
                  : () async {
                      ...
                      // IMPORTANT:
                      // dialogContext is the correct
                      // context to check after await.
                      if (!dialogContext.mounted) {
                        return;
                      }
